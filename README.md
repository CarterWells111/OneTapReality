...
OneTapReality｜一触如初是一个本地优先的情侣旅行纪念册 App。用户主动选择照片，填写旅行信息，获得可编辑的旅行册草稿，并将其保存在设备中；用户也可明确发布一份 NFC 礼品共享相册给受邀成员。
...
- 原生构建中的 NFC 礼品初始化、邮箱验证、共享相册与访问控制
...
本地旅行册默认仅保存在设备 SQLite 中，不自动同步，也不使用图像识别、分析埋点、支付或网络 AI。用户登录不会上传本地旅行册；只有礼品管理者明确发布 NFC 礼品时，礼品访问邮箱、共享快照和所选照片会保存到私有 R2 媒体存储，供已授权成员查看。礼品管理者可停用礼品并删除共享内容；“删除所有本地数据”不会停用已发布礼品。详见 [安全与隐私](./docs/SECURITY.md) 与 Alpha [运行手册](./docs/operations/ALPHA-STAGING.md)。
