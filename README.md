...
本地旅行册保持本机优先。NFC 礼品使用邮箱验证码账户：只有拥有者明确点击发布时，所选相册快照和照片副本才上传至私有 R2，并仅向至多三位获授权邮箱提供短期访问 URL。真实 NFC 写入仅支持 Development Build 或生产构建；完整的安全与部署说明见 [NFC 交接](./docs/NFC-HANDOFF.md)、[隐私说明](./docs/release/PRIVACY.md) 与 [安全说明](./docs/SECURITY.md)。

除 NFC 礼品拥有者明确发布的共享快照外，旅行册内容仍保留在本机 SQLite。应用不自动同步、不识别人脸或地点、不使用分析埋点、支付、订单或网络 AI。礼品账户仅用于邮箱验证、权限判断和成员管理；私有照片通过短期 R2 URL 发布或读取，具体边界见 [docs](./docs)。

Alpha 应使用与正式环境完全隔离的 staging 数据库、私有 R2 bucket、凭据与 `GIFT_URL_ORIGIN`。测试卡仅写入 staging 域名，正式卡仅写入正式域名，避免测试卡和测试数据访问真实礼品。应用内“删除所有数据”仅删除设备本地内容；已发布礼品的共享快照和媒体需在礼品管理流程中停用删除。
