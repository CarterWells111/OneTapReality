...
owner 完成实体礼品认领后，邀请 viewer 或 editor 会向对应邮箱发送通知。受邀者使用匹配邀请邮箱的统一账号登录，即可在 App 内看到礼品和完整相册预览，无需接触实体礼品；相册尚未发布时只显示待发布状态，不返回页面或媒体。

历史成员激活记录和接口仅为旧客户端兼容保留，不再参与授权。删除成员、切换权限、礼品停用和 `GIFT_SHARING_ENABLED` 停测开关仍会立即阻止不再允许的读取或编辑。
