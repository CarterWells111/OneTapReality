...
- 将 `engines.node` 从开放式下限改为 `">=20.16.0 <21"`。Railpack 优先读取该字段选择 Node，允许受支持的 Node 20 小版本，同时阻止自动切换至 Node 21 或 22。
- 新增 `devEngines`：
  - `runtime.name` 为 `node`，版本为 `">=20.16.0 <21"`，不匹配时 `error`。
...
- 新增配置测试，读取真实 `package.json`，断言 `packageManager`、`engines` 与 `devEngines` 保持一致。
...
本次只调整开发与部署工具链约束，不修改依赖版本、运行时业务代码、数据库、API 契约、客户端行为或 Railway 服务变量。
