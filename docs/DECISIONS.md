...
## 2026-09-11：受邀成员凭邮箱账号直接访问云端相册

一件实体礼品只需由拥有者通过 NFC 礼品入口完成认领并进入 `bound` 状态。此后 owner 邀请 `viewer` 或 `editor` 时，服务端向受邀邮箱发送通知；对方使用该邮箱完成验证码登录后，即可在 App 的“我的纪念品”查看相册并获得对应角色权限，不需要持有、扫描或再次触碰实体礼品。邮箱登录即完成对受邀邮箱的控制证明，不另设接受按钮或成员级 NFC 激活门槛。

- 未发布相册仍会显示在受邀列表中，但在 owner 首次发布前没有页面或媒体可读。
- 成员移除、角色调整、礼品停用和 `GIFT_SHARING_ENABLED` 停测开关继续即时生效；editor 发布及管理申请在开始与提交阶段仍重新检查当前成员角色。
- 历史 `gift_member_activations` 数据与旧激活接口仅为已发布客户端兼容保留，不再参与列表、读取、编辑或管理授权。
- 邀请通知复用现有服务端 Resend 配置，不增加客户端秘密、第三方服务、支付或分析；staging 继续受 `ALPHA_ALLOWED_EMAILS` 限制并与 production 隔离。

本决策取代 2026-08-16 关于“每位受邀成员必须持有礼品 token 并完成首次 NFC 激活”的访问规则；礼品拥有者的首次实体认领要求保持不变。
...
## 2026-08-31：旅行册编辑器可见编辑与可用素材修复

本轮只修复本地旅行册画布的编辑体验和离线贴纸素材，不改数据模型、SQLite schema、路由、账号、云端礼品共享、真实 NFC、支付、分析或第三方服务。文字元素在被选中后再次双击应直接进入输入态；“添加文字”和“添加照片”作为同一添加组并列在素材栏。字体、字号和颜色调整改为画布外的稳定内联编辑区，调整过程中画布持续可见且面板不重新挂载闪烁。文字缩放统一采用等比的画布变换与持久化字体比例，角落手柄只改文本框边界，不再让文字在框内出现跳变。贴纸继续完全打包在本地；保留既有贴纸 ID 以兼容已保存旅行册，并补充按明确用途命名的可用主题贴纸。Jest 同时限定为当前 `src` 和 `__tests__` 测试根目录，避免执行本机其他未合并工作树的副本。
...
## 2026-09-02：中国大陆访问可用性与生产数据保持区域迁移
...
## 2026-09-01：官网第一批 iOS 外部内测说明页

- 官网新增独立 `/beta/` 静态页面，并从首页主要导航提供入口，用于说明第一批 iOS 用户外部测试进度、测试重点、反馈要求和微信群加入方式。
- 官网可将 NFC 功能、第一批 NFC 纪念品、贴纸/相框/背景素材、城市打卡与足迹地图描述为当前测试范围内已实现能力，并移除这些能力对应的“未来规划”或“尚未实现”标识。
- 购买、订单、配送、支付、个人旅行数据统计、城市花束商品与文旅合作愿景仍未开放，官网必须继续明确标识；本阶段不新增上述能力。
- 微信群二维码使用用户提供的完整图片并作为官网本地静态资源发布；不新增远程图片、运行时请求、分析、支付或第三方服务。二维码有效期与更新方式按图片和页面文字明确说明。
- 外部测试页不得暗示 App Store 正式公开发布；NFC 成功仅指当前测试范围内完成验证，不能扩大为所有设备、标签或生产环境保证。完整设计见 `docs/superpowers/specs/2026-09-01-website-external-beta-page-design.md`。

## 2026-09-05：Expo SDK 57 与团队 Expo Go 对齐

- 客户端与 Expo 工具链从 SDK 54 升级到稳定版 SDK 57，对齐 React Native 0.86、React 19.2.3 以及团队成员已更新的 Expo Go。
- SDK 57 的最低本地与 CI Node 版本调整为 22.13.0；`package.json` 继续作为运行时最低版本的唯一来源，质量门禁同步验证该最低线。
- 项目继续使用 Continuous Native Generation；仓库不新增 `ios/` 或 `android/` 目录，后续 development build / EAS Build 由 Expo 重新生成原生工程。
- SDK 57 的 `eslint-config-expo` 新增 React Compiler 纯度诊断；现有 ref 与 effect 模式暂时保持原行为并显式关闭四条新增规则，后续按功能逐项测试迁移，不在框架升级中批量重写状态管理。
- 本次仅升级框架、兼容依赖与必要配置，不改变账号、礼品共享、NFC、数据库或 API 行为，不新增支付、分析、第三方服务、远程素材或客户端秘密。

## 2026-09-05：SDK 57 依赖与供应链安全收敛

- 在保持 Expo SDK 57 官方兼容版本矩阵的前提下，升级仍在兼容范围内的直接依赖；React Native、React、Expo 模块及相关测试工具不越过 SDK 57 指定版本。
- 对上游尚未发布新顶层版本、但已有兼容安全补丁的传递依赖使用精确 `overrides`，并通过干净安装、全量测试和生产导出验证。
- 不采纳 `npm audit fix --force` 建议的 Expo SDK 46/55 降级，也不为清除工具链误报切换到 Drizzle beta；无法在 SDK 57 内安全消除的上游审计项予以记录并等待上游发布。
- 本次不改变产品行为，不新增第三方服务、分析、支付、远程素材或客户端秘密。

## 2026-09-06：新建相册照片渐进载入

- 新建相册批量选图返回后立即显示与所选数量一致的本地缩略图槽位和载入进度；每张照片完成解码后独立显示，不等待整批照片同时完成。
- 照片全部成功载入后才挂载整册排版配置并允许生成草稿，避免大量排版预览与首屏缩略图竞争渲染资源；单张载入失败时明确提示重新选择并阻止生成不完整相册。
- 批量选择本地原图不再要求系统选择器额外执行 `0.8` 质量压缩；本地原图质量保持不变，既有 PDF 导出与礼品发布衍生图质量策略不变。
- 本次不新增依赖、网络请求、远程素材、照片内容分析、支付、分析或客户端秘密。

## 2026-09-10：管理员准备 NFC 卡时安全覆盖非产品记录

- `Prepare blank card` 可在同一次 NFC 会话中覆盖空记录、无法识别的 NDEF 记录及外部 URL，并写入当前构建环境的 activation URL；NDEF 写入本身替换整条消息，无需额外清空扫描。
- 只要现有记录包含可识别的 OneTapReality 标记（包括产品 HTTPS URL 与 `onetapreality:` 深链），准备操作必须停止且不写卡；礼品初始化仍只允许把当前环境的精确 activation URL 替换成已校验的礼品 URL。
- 写入后继续在同一次会话读回验证，不记录或上传卡片内容，不新增依赖、第三方服务或客户端秘密。
- 管理员卡片详情表单使用系统键盘 inset 调整，使名称、备注与保存操作在 iOS 键盘打开时仍可滚动到可见区域；不改变字段、数据或 API 行为。

## 2026-09-05：管理员 NFC 卡片固定编号与资料编辑

- 本变更使用迁移 `0015_gift_card_display_numbers`，追加在主线与 staging 已应用的 `0011`–`0014` 之后；不得复用或改写现有迁移编号。
- `gift_cards` 新增数据库分配的 `display_number` 与可空 `name`；固定编号从 1 开始单调递增并永久不复用，旧卡在迁移时回填，新卡由数据库序列在并发创建时保证唯一。
- 管理员卡片列表和详情只展示固定编号、名称、备注与状态，不展示内部 `CARD-*` 编码；初始化响应增加固定编号，旧版待处理本地数据继续兼容且用户文案不得回显内部编码。
- 既有管理员详情区可编辑名称（最多 80 字、可空）与备注（最多 240 字、可空），支持保存与取消；空白输入归一为 `null`，保存失败保留输入。
- 新增管理员 `PATCH /api/admin/gift-cards/:id`；所有卡片状态均可更新元数据，成功后写入 `metadata_updated` 审计事件，事件元数据只记录本次提交的字段名，不记录字段值或卡片编码。
- 列表搜索统一匹配固定编号、名称或备注；保留 NFC 写入、激活、认领、停用和 staging/production 隔离，不新增依赖、第三方服务、支付、分析或客户端秘密。

## 2026-09-20：新建纪念册照片增减与逐页配置

- 初次选择照片后允许继续追加与单张移除；保留已有有效分配、模板与加载状态，取消选图不改变内容。
- 照片排版默认逐页配置，同时保留一起配置入口；逐页可配置模板、照片数量与模板槽位顺序，并预览结果。
- 配置及生成草稿保持照片完整且不重复，遵守每页数量上限；所有操作使用本地素材，不新增依赖或服务。

## 2026-09-20：已有相册多选照片与整册重新配置

- 编辑相册的“照片与模板”支持按当前页剩余容量多选追加本地照片，并提供明确的模板槽位顺序调整操作；继续遵守照片持久化与取消回滚边界。
- 新增“再次编辑全部页面”单按钮入口，复用新建草稿的逐页配置能力；配置先暂存，应用后进入现有撤销、恢复草稿及正式保存流程，取消不修改相册。
- 保留现有页面身份、文字、贴纸、背景、封面及照片身份/裁剪，未修改的页面布局保持原样；不新增依赖、服务、上传或客户端秘密。

## 2026-09-20：第一批内测中“虚”负责的本地体验修复

- 地图来源文字留在地图外的独立位置，保持来源可见且不遮挡地图。
- 城市列表采用有序配色；未配置专属水彩图的城市提供有城市辨识度的本地插画，素材不依赖网络。
- 新建草稿与未正式保存的相册编辑可以在本机续编；草稿箱最多展示最近四份新建草稿，新增时移走最旧草稿。进入有未保存编辑的同一相册时由用户选择是否恢复。
- 页面贴纸等组件可放大到略大于页面，并保持有限的尺寸上限，避免手势产生无界数值。
- 不新增依赖、远程素材、云端同步、分析或客户端秘密。
