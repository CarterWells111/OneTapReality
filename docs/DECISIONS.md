...
## 2026-07-24：热门城市插画档案卡

“城市档案”在客户端本地展示至多五座城市：已有有效旅行记忆的城市按各自最新 `createdAt` 倒序优先，剩余位置依次以上海、深圳、杭州、南京、北京、香港补齐；第六张固定为“查看更多城市”入口。上海、深圳、杭州、南京、北京与香港使用随应用打包的无文字纸本水彩插画，其余城市使用代码绘制的统一线描占位。香港作为本地静态城市注册项加入城市内容、创建选择、统计、路由和地图固定标记。此范围不新增网络请求、服务、登录、支付、分析、真实 NFC、密钥或依赖。
