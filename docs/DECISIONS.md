...
个人页现链接到原生的、本机优先的隐私声明页，集中说明既有的本机 SQLite、照片 URI 与旅行册内容存储方式，`DemoDraftGenerator` 的限制，以及模拟 NFC 的当前状态；该页也提供经用户确认后删除本地记忆的既有操作。本次只澄清和集中呈现现有行为，不新增账户、云端或网络、模型或 AI 调用、真实 NFC、支付、数据 schema 或迁移行为。
