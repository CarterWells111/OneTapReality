...
| R2 staging bucket | 独立私有 bucket + 最小权限凭据 | 已完成（连通性待 `verify-r2` 确认） | 发布负责人 | bucket 名称 `onetapreality-staging`；因初建在非域名账号，已于 2026-08-06 在域名账号重建并轮换 API token；本地 `node scripts/verify-r2.cjs` 通过后回填结果 |
