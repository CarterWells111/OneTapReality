...
## 2026-09-07 故障复盘与后续观察规则

production 曾因实际发布流程未执行迁移而停在 Schema 14，已补齐至 16，并在实际 Railway 服务启用预部署迁移、健康门禁和等待 CI。详见 [部署记录](DEPLOYMENT-LOG.md#2026-09-07production-schema-14--16-与发布门禁修复) 和 [每次发布检查表](../EXECUTION-CHECKLIST.md#每次后端发布的-schema-与生效配置门禁)。

- 这是一份有时间戳的修复证据，不能替代当日探测；最低 Schema 仍读取当日远端 main，不固定为 16。
- production 当前域名对应 `migration-sg-rehearsal / TapMigrationServer`；平台 Online 或部署成功不替代公开健康结果。无权限核对实际配置时标“待人工确认”。
- 使用产品标识 User-Agent 和 JSON Accept 访问公开健康接口。403 记录为访问层阻断、数据库无法验证，不能猜测数据库损坏或以零值填充；不自动修改 Cloudflare 安全规则。
- staging 相册上传/发布成功与小时级维护成功分开记录；不能把维护错误自动归因用户上传调试，也不能从健康恢复推断维护已恢复。
- 本次生产人工修复授权不扩展自动任务权限；仍禁止自动迁移、维护 POST、production 数据库访问与云端写操作。EAS 上传/构建另行确认。
