...
第一阶段稳定观察至少 7 天。期间检查维护状态、死信数量、请求延迟和错误码，不记录原始 IP、邮箱、礼品 token、对象 key 或 secret。第二阶段尚未加入 Drizzle 自动迁移序列，防止 Railway pre-deploy 提前执行破坏性操作。

满 7 天后必须再次分别批准生产只读检查、本地 `pg_dump`、本地恢复验证、约束验证、移除遗留运行代码、删除 `gift_email_codes` / `gift_sessions` 的 migration、Railway 部署与首次维护调用。只有确认无运行引用并验证备份可恢复，才生成第二阶段 migration；禁止修改已应用的历史 migration。

## 回滚

- Worker 异常：禁用 Cron，不删除 Worker Secret 以外的数据，继续依赖请求兜底。
- API 异常：回滚应用代码；第一阶段 schema 保持向后兼容，不回滚增量列和索引。
