...
- owner 首次认领仍需实体礼品；之后邀请 `viewer` 或 `editor` 会发送邮件，受邀者使用匹配邀请邮箱登录即可在 App 内查看，无需接触实体礼品。
- `viewer` 保持只读，`editor` 保持完整 Canvas 编辑与新版本发布权限。读取、发布和管理申请继续实时检查成员、角色、举报隐藏、礼品状态与停测开关。
- `gift_member_activations` 与激活端点仅为旧客户端兼容保留，不再是列表、读取、编辑或管理授权条件。本节取代上方 2026-08-16 的成员级激活规则。
