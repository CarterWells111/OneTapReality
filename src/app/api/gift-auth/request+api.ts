...
import { createGiftEmailCode as persistGiftEmailCode, isGiftEmailCodeRateLimited } from "../../../server/gifts/repository";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const code = await createGiftEmailCode(email, pepper, undefined, now);
    const db = getServerDatabase();
    if (await isGiftEmailCodeRateLimited(db, code.email, new Date(nowDate.getTime() - 15 * 60 * 1000).toISOString())) throw new ApiError(429, "email_code_rate_limited", "Please wait before requesting another code");
    await persistGiftEmailCode(db, { id: crypto.randomUUID(), email: code.email, codeHash: code.codeHash, expiresAt: code.expiresAt, createdAt: now });
